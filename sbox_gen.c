...
// extern byte key[KEY_BYTES];
// extern byte** sboxm;
// extern byte AES_sbox[16][16];

byte key[KEY_BYTES] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15};
...
void Algorithm1(byte* sbox, byte key[KEY_BYTES]) {
...
void Algorithm2(byte** sbox, byte key[KEY_BYTES]) {

	byte* left = expand_bytes(key);
	byte* right = expand_bytes(&key[KEY_BYTES / 2]);
